...
const AnalyticsChart = ({ isDarkMode }) => {
  ...
  useEffect(() => {
    const fetchCharts = async () => {
      try {
        const res = await fetch(`http://localhost:3000/api/analytics/chart-data/${projectId}`);
        const data = await res.json();

        // Transform Hourly Volume (normalize to 0-100 for CSS height)
        const maxVal = Math.max(...data.hourlyVolume.map(h => h.count), 1);
        const normalizedVolume = data.hourlyVolume.map(h => (h.count / maxVal) * 100);
        ...
        const bgColors = { email: "bg-blue-500", inapp: "bg-indigo-500", sms: "bg-pink-500" };
        ...
        setChartData({ volume: normalizedVolume, channels: transformedChannels });
      } catch (err) {
            ...
            <div key={i} className="flex-1 h-full flex flex-col justify-end items-center group">
              ...
              >
                <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-all whitespace-nowrap z-50">
                   {/* Tooltip shows normalized value (approximate) */}
                   {Math.round(val)}% Intensity
                </div>
              </div>
              <span className="text-[10px] font-bold opacity-30 mt-2">H{i}</span>
            </div>
          ))}
        </div>
      </div>

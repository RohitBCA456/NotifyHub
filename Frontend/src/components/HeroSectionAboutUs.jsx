      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className={`text-lg md:text-xl max-w-3xl mx-auto leading-relaxed ${
            ...
            NotifyHub is the central nervous system for your technical stack. 
            We bridge the gap between your applications and your team.
          </p>
        </div>
        ...
        {/* Values Section */}
        <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center p-8 md:p-12 rounded-[40px] border ${
          isDarkMode ? "bg-slate-900/50 border-slate-800" : "bg-white border-slate-200 shadow-xl shadow-slate-200/50"
        }`}>
          <div>
            <h2 className={`text-3xl font-black mb-6 ${isDarkMode ? "text-white" : "text-slate-900"}`}>
              Why Teams Choose <br />NotifyHub?
            </h2>
          ...
          <div className="relative">
            <div className="aspect-video rounded-3xl bg-gradient-to-br from-blue-600 to-indigo-900 flex items-center justify-center p-8 shadow-2xl">
                <div className="text-white text-center">
                    ...
                    <p className="text-2xl font-bold italic">"Reliability in every packet."</p>
                </div>
            </div>
          </div>
        </div>
...
const StatCard = ({ icon, label, value, description, isDarkMode }) => (
  <div className={`p-8 rounded-[32px] border transition-all hover:-translate-y-1 ${
    isDarkMode ? "bg-slate-900 border-slate-800 hover:bg-slate-800/80" : "bg-white border-slate-200 shadow-sm hover:shadow-xl"
  }`}>
    <div className={`w-14 h-14 rounded-2xl mb-6 flex items-center justify-center ${isDarkMode ? "bg-slate-800 text-blue-400" : "bg-blue-50 text-blue-600"}`}>
      {icon}
    </div>
    <p className={`text-4xl font-black mb-2 ${isDarkMode ? "text-white" : "text-slate-900"}`}>
      {value}
    </p>
    <p className={`text-sm font-bold uppercase tracking-widest mb-2 ${isDarkMode ? "text-blue-400" : "text-blue-600"}`}>
      {label}
    </p>
...
const ValueItem = ({ icon, title, text, isDarkMode }) => (
    <div className="flex gap-4">
        ...
        <div>
            <h4 className={`font-bold mb-1 ${isDarkMode ? "text-white" : "text-slate-900"}`}>{title}</h4>
            <p className={`text-sm ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>{text}</p>
        </div>
    </div>
)

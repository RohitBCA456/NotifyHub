...
const NotificationFeed = ({ isDarkMode, currentUserId }) => {
  ...
  useEffect(() => {
    ...
    socket.on("notification_sent", (data) => {
      setFeedItems((prev) => {
        // Check if notification already exists (e.g., updating Processing to Sent)
        const exists = prev.find(item => item._id === data._id);
        
        if (exists) {
          return prev.map(item => item._id === data._id ? { ...item, ...data } : item);
        }
        // If it's a new notification, add it to the top of the feed
        return [data, ...prev].slice(0, 15); // Keep last 15 items
      });
    });
          ...
          <div key={item.notificationId} className={`flex flex-col md:flex-row md:items-center justify-between p-5 rounded-2xl border transition-all ${
            ...
            <div className="flex items-center gap-4 mb-4 md:mb-0">
              <div className={`p-2.5 rounded-xl ${isDarkMode ? "bg-slate-900 text-blue-400" : "bg-white text-blue-600 shadow-sm"}`}>
                ...
                {item.channel?.toLowerCase() === "email" ? <Mail size={18} /> : 
                 item.channel?.toLowerCase() === "webhook" ? <Globe size={18} /> : 
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`flex items-center gap-1.5 text-[10px] font-bold px-3 py-1 rounded-full ${
      ...
      isProcessing ? "bg-blue-500/10 text-blue-500" : "bg-red-500/10 text-red-500"
    }`}>
      {isProcessing ? <Loader2 size={12} className="animate-spin" /> : 
       isSent ? <CheckCircle2 size={12} /> : 
       <AlertCircle size={12} />}
      {status?.toUpperCase()}
    </span>
  );

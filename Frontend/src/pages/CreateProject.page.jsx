...
const CreateProjectPage = () => {
  ...
  const handleCreate = () => {
    ...
    // Simulate Server Provisioning (1.5s delay)
    setTimeout(() => {
  ...
  return (
    <div className={`pt-24 pb-20 transition-colors duration-300 min-h-screen flex items-center ${
      ...
      {step === 1 && (
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 w-full animate-in fade-in duration-500">
          <div className="mb-12 text-center">
            ...
            <p className="text-lg opacity-70">Define your workspace and select your notification channels.</p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className={`p-8 rounded-3xl border transition-colors ${
              ...
              <div className="grid grid-cols-1 gap-4">
                <PreferenceToggle icon={<Mail size={22} />} title="Email" active={preferences.email} onClick={() => handleToggle('email')} isDarkMode={isDarkMode} />
                <PreferenceToggle icon={<Smartphone size={22} />} title="In-App" active={preferences.inApp} onClick={() => handleToggle('inApp')} isDarkMode={isDarkMode} />
                <PreferenceToggle icon={<MessageSquare size={22} />} title="SMS" active={preferences.sms} onClick={() => handleToggle('sms')} isDarkMode={isDarkMode} />
              </div>
            </div>
  ...
  <button 
    ...
    className={`flex items-center gap-5 p-5 rounded-2xl border text-left transition-all ${
      active 
        ...
        : (isDarkMode ? "bg-slate-800 border-slate-700 hover:border-slate-600" : "bg-white border-slate-200 hover:border-slate-300 shadow-sm")
    }`}
  >
    <div className={`p-3 rounded-xl transition-colors ${
      active ? "bg-blue-600 text-white shadow-lg" : (isDarkMode ? "bg-slate-700 text-slate-400" : "bg-slate-50 text-slate-400 border border-slate-100")
    }`}>
      {icon}
    </div>
    <div className="flex-1">
      <p className={`text-lg font-bold ${isDarkMode ? "text-white" : "text-slate-900"}`}>{title}</p>
    </div>
    <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all ${
      active ? "bg-blue-600 border-blue-600" : (isDarkMode ? "border-slate-600" : "border-slate-300")
    }`}>
